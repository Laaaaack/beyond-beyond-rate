...
def generate_spike_train(f, isi, num_neurons=10, T=1000, max_attempts=100):
    """
    ISI 模式 (支持小数 firing rate f):
    - 每个神经元独立发放
    - f 是 Hz，允许小数
    - 每个 pair 由两个 spike 构成，间隔 isi
    """
    spike_trains = np.zeros((num_neurons, T), dtype=np.uint8)

    duration_s = T / 1000.0
    pairs = int(np.round(f * duration_s))   # 期望的 pairs 数（支持小数 Hz）
    total_spikes = 2 * pairs
