"""Input Per-Spike Jitter — SHD (perturbation-aware training).

For each jitter level sigma, the train/val/test splits are regenerated with
per-spike Gaussian jitter applied to the input spike trains, and a fresh model
is trained on that jittered dataset and evaluated on the correspondingly
jittered test set. This is the perturbation-aware counterpart of the
train-once / eval-all jitter experiment, and follows the logic of the original
Beyond Rate notebook
...
nearest unoccupied time bin. A spike that finds no free bin within
max_attempts stays where it was, so spike counts per neuron are approximately
preserved while precise timing is destroyed.
...
# The .mat files live with the realistic SHD experiment; share them rather
# than keeping a second copy here.
DATASET_CONFIGS = {
    "whole": {
        "mat_file": "../../realistic/shd/shd_data/shd_whole.mat",
        "input_dim": 700,
    },
    "part": {
        "mat_file": "../../realistic/shd/shd_data/shd_part_new.mat",
        "input_dim": 224,
    },
    "norm": {
        "mat_file": "../../realistic/shd/shd_data/shd_norm_new.mat",
        "input_dim": 224,
    },
}
...
# Input-jitter sweep: sigma values in ms
...
def jitter_spike_train(
    ...
    """Shift every spike by an independent Gaussian offset.

    Args:
        spike_train: Binary array of shape (num_neurons, T).
        sigma: Jitter standard deviation in ms (0 leaves the train unchanged).
        max_attempts: Max tries to find a free time bin per spike. A spike that
            fails every attempt keeps its original time.
...
def preprocess_full_dataset(
    ...
    """Split the dataset and jitter the input spikes of every split at sigma.

    Args:
        X: Spike trains of shape (N, num_neurons, T).
        Y: Integer labels of shape (N,).
        sigma: Jitter standard deviation applied to all splits.
        ...
        seed: Seed for the split shuffle and the jitter RNG.
    ...
    def _process(indices: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        X_out = np.array([jitter_spike_train(X[i], sigma) for i in indices])
...
class JitterSHDNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Jitter is baked into the data upstream, so this is a plain forward
        pass with no perturbation hook.
        """
...
def build_loss_and_optimizer(net: JitterSHDNetwork, lr: float = 0.1) -> tuple:
...
def train_model(
    ...
    """Train a JitterSHDNetwork on already-jittered training data.

    Args:
        X_train: Jittered training spike trains, shape (N, neurons, T).
        Y_train: Training labels.
        X_val: Jittered validation spike trains.
...
def test_accuracy(
    ...
    """Compute classification accuracy on a pre-jittered test set.

    Args:
        net: Trained network.
        X_test: Jittered test spike trains.
        Y_test: Test labels.
...
# =====================================================================
# Sweep over input-jitter levels
# =====================================================================
...
def run_variation_sweep(
    ...
    """Input-jitter sweep for one (dataset_key, use_delay) variation.

    Loads the dataset, then for every sigma in SIGMA_VALUES regenerates the
    jittered splits, trains a fresh model, evaluates it on the jittered test
    set, and persists the per-variation checkpoints and JSON.
    ...
    Returns:
        Tuple of (test accuracy per sigma, training log per sigma).
    """
    ...
    model_prefix = f"jitter_{dataset_key}_{delay_tag}_inputpert"
    ...
    for sigma in SIGMA_VALUES:
        print(f"\n===== {model_prefix} | Input jitter sigma = {sigma} ms =====")

        print("Preprocessing dataset (jittering input spike trains)...")
        ...
        print(f"Test accuracy at sigma={sigma}: {acc:.4f}")

        sweep_results[sigma] = {"accuracy": float(acc)}
        training_logs[f"sigma={sigma}"] = log

        model_path = os.path.join(
            data_dir, f"{model_prefix}_sigma{int(sigma):02d}.pt"
        )
...
def save_results(
    ...
    """Write sweep accuracies and per-sigma training logs to the log directory.

    Args:
        sweep_results: Test accuracy per jitter level.
        training_logs: Per-epoch training log per jitter level.
    ...
    results_serialisable = {
        str(sigma): {"accuracy": float(data["accuracy"])}
        for sigma, data in sweep_results.items()
    }
    ...
    logs_serialisable = {
        ...
        for sigma_key, log in training_logs.items()
    }
...
def main() -> None:
    """Run the configured input-jitter sweep(s)."""
    if TRAIN_ALL_VARIATION:
        ...
        for dataset_key, use_delay in ALL_VARIATIONS:
            ...
            print(f"  - jitter_{dataset_key}_{tag}_inputpert")
        ...
        print(f"Model prefix: jitter_{DATASET_KEY}_{tag}_inputpert")
        print(f"sigma sweep: {SIGMA_VALUES}")
