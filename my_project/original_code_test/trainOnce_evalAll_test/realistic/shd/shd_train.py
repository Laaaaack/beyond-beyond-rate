"""SHD — Input-Layer Perturbation (train-once / eval-all).
...
by applying spike-timing perturbation to the *input* spike trains across a
sweep of perturbation levels f. This mirrors the original Beyond Rate
input-perturbation experiments but follows the train-once / eval-all protocol:
a single clean-trained model is evaluated at every f, rather than training one
model per f.
