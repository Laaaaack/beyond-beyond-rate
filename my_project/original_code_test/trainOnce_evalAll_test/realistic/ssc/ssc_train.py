"""SSC — Input-Layer Perturbation (train-once / eval-all).

Train a 2-hidden-layer SLAYER SNN on the Spiking Speech Commands (SSC)
dataset with clean (unperturbed, f=0) inputs, then evaluate the frozen model
by applying spike-timing perturbation to the *input* spike trains at test
time, across a sweep of perturbation levels f. This mirrors the original
Beyond Rate input-perturbation experiments but follows the train-once /
eval-all protocol: a single clean-trained model is evaluated at every f,
rather than training one model per f. Supports the whole/part/norm dataset
variants and SGD (no delay) / SGD-delay modes.
...
class SSCNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # they reach this method, so training and evaluation share one clean
        # network.
