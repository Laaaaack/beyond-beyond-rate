"""Input Per-Neuron Jitter (Shift) — SHD (train-once / eval-all).

We train a single 2-hidden-layer SNN on the SHD dataset with clean
(unperturbed) inputs, then sweep per-neuron Gaussian shift applied to the
*input* spike trains during evaluation only. This is the input-site
counterpart of the hidden-layer shift experiment and mirrors the original
Beyond Rate input-perturbation notebooks
(temporal_shd_project/code/pertubation/draft/shift/slayer_shift_part.ipynb),
but follows the train-once / eval-all protocol instead of training a separate
model per shift level.
