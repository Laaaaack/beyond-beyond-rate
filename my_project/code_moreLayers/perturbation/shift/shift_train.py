"""Deeper SHD — 4-Hidden-Layer Per-Neuron Shift (train-all / eval-all).

The 4-hidden-layer counterpart of ``code/perturbation/shift/shift_train.py``.
The network stacks four spiking hidden layers (``h1``..``h4``) followed by a
spiking readout, and a per-neuron Gaussian shift can be injected at the output
of any one hidden layer, selected by ``perturb_layer in {1, 2, 3, 4}``.

A per-neuron shift draws a single Gaussian offset per neuron and applies it to
all of that neuron's spikes, preserving intra-neuron ISI structure while
disrupting cross-channel timing. For each (perturbation site, shift level
``sigma``) a fresh model is trained end-to-end with the shift active at that
site on every batch (through a straight-through estimator), then evaluated at
the same ``sigma``. Sweeping the site across all four hidden layers yields a
depth trajectory of temporal sensitivity (see ``code_moreLayers/README.md``).

SHD data is loaded from ``../../realistic/shd/shd_data``. Everything except
depth and perturbation site matches the 2-hidden-layer shift script.
"""
...
def shift_hidden_batch(
    ...
    (logical OR), matching the numpy reference. All operations stay on the
    ...
    # One integer offset per (batch, neuron); broadcast over the time axis.
...
class ShiftSHDNetwork(nn.Module):
    """4-hidden-layer SLAYER SNN with configurable per-neuron shift.
    ...
    ``forward(x, sigma, perturb_layer)`` injects a per-neuron Gaussian shift of
    ...
    def _apply_shift(
        ...
        """STE wrapper around ``shift_hidden_batch``.

        Forward value is the shifted tensor; backward gradient is the
        ...
        Args:
            ...
            sigma: Per-neuron shift std dev in ms.

        Returns:
            Shifted tensor that still carries the original gradient.
        """
        if sigma <= 0:
            return hidden
        shifted = shift_hidden_batch(hidden, sigma)
        return hidden + (shifted - hidden).detach()
    ...
    def forward(
        ...
        """Forward pass with a per-neuron shift at one hidden-layer site.

        Args:
            x: Input spike trains, shape (B, C, T) or (B, C, 1, 1, T).
            sigma: Per-neuron shift std dev (ms) applied at the selected site.
            perturb_layer: Hidden layer whose spike output is shifted
                (1..num_hidden_layers).
        ...
        for i in range(self.num_hidden_layers):
            ...
            # Shift this layer's spike output directly, before its delay.
            if (i + 1) == perturb_layer:
                h = self._apply_shift(h, sigma)
...
def train_model(
    ...
    """Train a ShiftSHDNetwork with a per-neuron shift at one hidden-layer site.

    Args:
        train_loader: Training DataLoader.
        val_loader: Validation DataLoader.
        input_dim: Number of input neurons.
        perturb_layer: Hidden layer shifted during training (1-indexed).
        sigma: Per-neuron shift std dev (ms) applied at the selected site.
...
def test_with_shift(
    ...
    """Evaluate accuracy with a shift at ``perturb_layer`` and level ``sigma``."""
    net.eval()
    correct = 0
    total = 0
...
def test_with_repeats(
    ...
    """Repeat ``test_with_shift`` for mean ± std error bars."""
    ...
    for repeat in range(num_repeats):
        ...
        accuracies.append(
            test_with_shift(net, test_loader, perturb_layer, sigma=sigma)
        )
    return {
...
def run_variation_sweep(
    ...
    sigma in SIGMA_VALUES with the per-neuron shift active at ``perturb_layer``
    during training, evaluates each at the same sigma, and persists
    per-variation checkpoints + JSON.

    Args:
        perturb_layer: Hidden layer shifted during this sweep (1-indexed).
    ...
    model_prefix = f"shift_perturb{perturb_layer}_{dataset_key}_{delay_tag}"

    print(f"\n{'#' * 70}")
    print(
        f"# Layer-{perturb_layer} shift: dataset={dataset_key} | delay={delay_tag}"
    )
    ...
    models: dict[int, ShiftSHDNetwork] = {}
    logs: dict[int, dict] = {}
    results: dict[int, dict] = {}

    for sigma in SIGMA_VALUES:
        print(f"\n=== Training {model_prefix} with hidden shift sigma={sigma} ms ===")
...
def main() -> None:
    ...
    for perturb_layer in PERTURB_LAYERS:
        for ds, ud in variations_to_run:
            ...
            print(f"  - shift_perturb{perturb_layer}_{ds}_{tag}")
