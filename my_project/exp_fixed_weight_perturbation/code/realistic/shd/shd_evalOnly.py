"""Experiment 2A: SHD - Hidden-Layer Perturbation (evaluation only).

Load pre-trained 2-hidden-layer SLAYER SNN checkpoints from `data/` and sweep
spike-timing perturbation applied at the output of the 1st hidden layer. No
training is performed.

The network definition and test split mirror shd_train.py, so the checkpoints
that script produces load without modification and are evaluated on the same
held-out samples they were validated against.
...
# Data split ratios. Must match shd_train.py, or the "test" set would overlap
# the data the checkpoints were fitted on.
TRAIN_RANGE = (0.0, 0.6)
VAL_RANGE = (0.6, 0.75)
TEST_RANGE = (0.75, 0.9)
...
def build_test_loader(
    ...
    Uses the same contiguous TEST_RANGE slice as shd_train.py's
...
class SHDNetwork(nn.Module):
    ...
    Structurally identical to the network in shd_train.py so that its
...
def load_trained_model(
    ...
    Args:
        checkpoint_path: Path to a state_dict saved by shd_train.py.
