"""Experiment 2B: SSC — Hidden-Layer Perturbation (evaluation only).

Load pre-trained 2-hidden-layer SLAYER SNN checkpoints from `data/` and sweep
spike-timing perturbation applied to the output of the 1st hidden layer at test
time. No training is performed. Supports the whole/part/norm dataset variants
and SGD (no delay) / SGD-delay modes.

The network definition and perturbation hook mirror ssc_train.py, so the
checkpoints that script produces load without modification. The train/val/test
split uses the seeded shuffle from ssc_2ndLayer_train.py: the `whole` dataset is
stored in sorted class-blocks, so a contiguous split would yield skewed,
partially-missing classes.
"""
...
def load_test_data(h5_path: str, target_T: int = 200) -> tuple:
    ...
    # Same seeded permutation the training script applies before splitting.
    np.random.seed(SEED)
...
class SSCNetwork(nn.Module):
    ...
    Structurally identical to the network in ssc_train.py so that its
    ...
    def _first_hidden(self, x: torch.Tensor) -> torch.Tensor:
        # Input -> PSP -> fc1 -> spike. Returns raw binary hidden1 spikes
        # (pre-delay), so the perturbation hook sees a strictly 0/1 tensor.
    ...
    def forward_with_hidden_perturbation(
        ...
        """Run a forward pass, perturbing the 1st hidden layer's spikes.
...
def load_trained_model(
    ...
    Args:
        checkpoint_path: Path to a state_dict saved by ssc_train.py.
...
def test_with_hidden_perturbation(
    ...
    """Measure test accuracy with the 1st hidden layer perturbed.
