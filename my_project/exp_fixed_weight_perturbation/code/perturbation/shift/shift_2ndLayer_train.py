...
(unperturbed) inputs, then sweep per-neuron Gaussian shift at the 2nd hidden
...
class ShiftSHDNetwork(nn.Module):
    ...
    def _second_hidden(self, hidden1: torch.Tensor) -> torch.Tensor:
        ...
        # NOTE: delay1 is applied here (it lives at the start of the original
        # combined method, not at the end of _first_hidden), so the returned
        # tensor is the binary spike() output and is safe to perturb.
        if self.use_delay:
    ...
    def forward_with_hidden_perturbation(
        ...
        # Perturbation hook on the 2nd hidden layer's binary spike output,
        # before delay2 runs (delay2 would make the tensor fractional).
        if sigma > 0:
