"""Experiment 3C: Hidden Spike Deletion — 2nd Hidden Layer (eval-only).

Load 2-hidden-layer SNNs that were trained on the clean SHD dataset and sweep
per-spike deletion applied at the 2nd hidden layer output at evaluation time
only (train-clean / eval-perturbed protocol): each hidden spike is independently
dropped with probability ``p_d``. **No training is performed.**

This is the "train once / eval all" companion to ``deletion_2ndLayer_train.py``.
Instead of training a fresh model per deletion probability, it loads the single
clean-trained checkpoint ``deletion_2ndLayer_{dataset}_{delay_tag}_pd00.pt`` for
each configuration and evaluates that one model across the whole p_d sweep.
"""
...
class DeletionSHDNetwork(nn.Module):
    ...
    def _apply_deletion(
        ...
        """Apply spike deletion to the 2nd hidden layer output (eval-only)."""
        if p_d <= 0:
            return hidden
        return delete_hidden_batch(hidden, p_d)
...
def load_pretrained_model(
    ...
    """Load a clean-trained DeletionSHDNetwork checkpoint ready for evaluation."""
...
def run_variation_sweep(
    ...
    """Load the clean checkpoint and sweep 2nd hidden layer deletion at eval time."""
    ...
    ckpt_path = DATA_DIR / f"{model_prefix}_pd00.pt"

    print(f"\n{'#' * 70}")
    print(f"# 2nd Hidden Layer Deletion (eval-only): dataset={dataset_key} | delay={delay_tag}")
    ...
    results_path = LOG_DIR / f"{model_prefix}_evalOnly_deletion_sweep_results.json"
