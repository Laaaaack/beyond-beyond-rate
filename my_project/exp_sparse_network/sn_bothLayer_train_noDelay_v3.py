"""Both-layer milestone v3 (training), NO-DELAY arm: the a x s dissociation factorial.

Third sibling of [sn_train_noDelay_v3.py](sn_train_noDelay_v3.py) (penalties on the
**1st** hidden layer) and
[sn_2ndLayer_train_noDelay_v3.py](sn_2ndLayer_train_noDelay_v3.py) (penalties on the
**2nd**). Same network, data, splits, optimiser, schedule and logging;
the only difference is that both penalties are charged against **both hidden layers
...
baseline measurement says those are not the same question. Measured across v1's 15
no-delay checkpoints — where only layer 1 was ever penalised:

===============================  ==========  ==============================
correlation across v1's gradient  value       what it says
===============================  ==========  ==============================
``rho(a1, s1)``                   **-0.943**  layer 1's own confound: the one
                                              document 5 exists to break
``rho(a2, s2)``                   **+0.829**  layer 2 carries its own
                                              confound, running the *other*
                                              way
``rho(s1, s2)``                   **-0.829**  **the finding that motivates
                                              this script**
``rho(a1, a2)``                   **+0.961**  the two layers' activity levels
                                              are near-inseparable
``rho(a_net, s_net)`` pooled      **-0.879**  network-wide, the confound is
                                              nearly as bad as layer 1's
===============================  ==========  ==============================

Read the third row carefully. Under a **layer-1-only** penalty, the two layers' silent
fractions move in *opposite* directions: as layer 1 is driven toward silence, layer 2
becomes *less* silent. The layer-1 penalty does not make the network selective — it
trades layer 1's silence for layer 2's activity. Any claim of the form "sparser networks
do X", made from a grid that constrains one layer, is a claim about a network that was
sparsified in one place and compensated in another.

The fifth row is the consequence for the design: pooled over both layers, ``a`` and
``s`` are confounded at **-0.879**, well past the ``|rho| <= 0.5`` acceptance threshold.
So the factorial is as necessary at the network level as it was at layer 1, and it is a
**third** confound being broken.
...
``rho(a1, a2) = +0.961`` already is observationally. That is a property of the question,
...
2. **Floor — sets ``s``.** ``relu(THETA_MARGIN - max_t u(t))`` on the **membrane
   ...
   0.19-2.08 (layer 1) and 0.00-0.36 (layer 2) against ``theta = 10``; a count-based
...
----------------------------------------------
``k`` is set per layer rather than shared, because the two layers do not run at the same
natural rate and an equal *absolute* budget would not be an equal *relative* cut.
``CEILING_K`` lists layer 1's levels; layer 2's are ``CEILING_K_LAYER2_RATIO`` times
them. In this arm the measured natural ``a`` is 3.06-11.49 (layer 1) against 7.49-12.46
(layer 2) — close enough that the ratio is **1.0** and the two budgets coincide. The
delay arm, where layer 2 runs about twice as hard, uses 2.0. One row knob either way.

Where the floor's layer-2 reachability edge goes
-------------------------------------------------
``d potential2 / d W2 = psp(hidden1)`` is surrogate-free, **but it is exactly zero for
any sample whose entire layer 1 is silent** — such a pair is reachable only back through
``fc1``'s surrogate. Currently 0.00% of samples across all 27 v1 checkpoints.
...
-----------------------------------------------
**Measured for this arm, per layer** — ``NATURAL_SPIKES_PER_NEURON`` (7.85 at layer 1,
5.53 at layer 2), the per-layer natural ``a`` that sets ``CEILING_K_LAYER2_RATIO``, and
...
4. ``spikes_per_neuron`` not inflated beyond each layer's natural rate (7.85 / 5.53);
...
stop. The observational baselines this must improve on in this arm are **-0.943**
(layer 1), **+0.829** (layer 2) and **-0.879** (network).
...
run at both layers, at that layer's own window (``[0, 88)`` for layer 1, ``[0, 90)`` for
layer 2 in this arm). See:
...
- a checkpoint ``sn_data/sparse_whole_nodelay_v3L12_k{k}_floor{f}_seed{seed}.pt``;
...
- a row in ``sn_log/sparse_whole_nodelay_v3L12_train_summary.json`` recording clean test
...
USE_DELAY: bool = False       # no learnable delays
...
#
# Layer 1's natural `a` in this arm is 3.06-11.49 and layer 2's is 7.49-12.46 (measured
# on v1's 15 checkpoints), so k = 8 sits near the top of both ranges and k = 1 is an
# order of magnitude under both. The grid therefore spans the natural rate down to a
# single spike at each layer, which is what the row axis needs.
...
#
# Set from the measured natural `a` per layer in THIS arm: 3.06-11.49 at layer 1 against
# 7.49-12.46 at layer 2. The two ranges largely overlap, so an equal absolute budget is
# already an equal relative one and the ratio is 1.0. This constant is not decorative --
# the delay arm measures 3.82-15.47 against 20.71-30.44 and uses 2.0, and carrying this
# arm's value across would leave layer 2 cut about twice as hard as layer 1 there.
CEILING_K_LAYER2_RATIO: float = 1.0
...
# At layer 1, 3.0 (v2's value) proved too weak -- it left `a` spanning only 1.24-1.42x
# across k = 1 -> 8, because the penalty reaches equilibrium far above k and stops
# pushing -- and 10.0 roughly doubled the row-axis span at an accuracy cost that stayed
# usable.
#
...
#     SMALLER value;
#   - the layer-2 term's gradient reaches fc2 directly but fc1 only through the
#     surrogate, and at layer 1 even strength 10 left 41% of pairs above k at k = 1 --
#     an argument for a LARGER one.
...
# chance for all 60 epochs, unrecoverable; warmup = 20 survived. 50 bought nothing over
# 20 in this arm, so 20 was adopted.
#
...
# (d potential2 / d W2 = psp(hidden1) = 0), so the two failures compound. The probe's
...
# The natural firing rate of each hidden layer in this arm, measured on v1's 15
# checkpoints (layer 1 spans 1.21-7.85, layer 2 spans 3.52-5.53; these are the densest).
# The ceiling must not drive either layer ABOVE its own figure; v2.2 inflated layer 1
# from ~6.3 to 12.5-21.9 and stopped being an instance of "sparser-activity networks"
# as a result.
#
# These are per-LAYER and per-ARM figures. The delay arm measures 11.66 and 23.93, so
# the constant must never be carried across either boundary.
NATURAL_SPIKES_PER_NEURON: dict[int, float] = {1: 7.85, 2: 5.53}
...
# each level the acceptance check reports. Note that the three disagree in sign and
# magnitude -- layer 1's two axes oppose each other, layer 2's move together, and the
# network-wide pool inherits layer 1's sign at nearly its full strength.
...
# separating the layers is what the two single-layer grids are for. rho(s1, s2) is the
# one to watch: at -0.829 a layer-1-only penalty made the two layers' silence move in
# OPPOSITE directions, and this design should pull it positive.
V1_CROSS_LAYER_CORRELATION: dict[str, float] = {"a": +0.961, "s": -0.829}
...
class SparseSHDNetwork(nn.Module):
    """2-hidden-layer SLAYER SNN without delays, for the both-layer v3 factorial.

    The parameter set is identical to v1's no-delay class of the same name —
    ``fc1``/``fc2``/``fc3`` weight-norm parameters and nothing else — so these
    ...
    machinery in the whole model is the SRM neurons' own membrane dynamics.
    ...
    def _second_hidden(
        ...
        """hidden1 -> fc2 -> spike, returning the potential as well as the spikes.

        ``d potential2 / d W2 = psp(hidden1)`` is likewise an ordinary convolution
        gradient — no surrogate — with the one caveat that it is zero for a sample whose
        entire layer 1 is silent. In this design the layer-1 floor guards that
        precondition in the floor-on column; the floor-off column is where it is worth
        watching.
        ...
        """
        potential2 = self.fc2(self.slayer.psp(hidden1))
...
def floor_penalty(potential: torch.Tensor,
    ...
    0.19-2.08 at layer 1 and **0.00-0.36** at layer 2 against ``theta = 10`` in this arm
...
def report_design_acceptance(summary: dict[str, dict]) -> None:
    ...
    reading of the same grid is also interpretable.
    ...
    single-layer grids are for. ``rho(s1, s2)`` is the informative one: v1's
    layer-1-only penalty drove it to -0.829, meaning the two layers' silence moved in
    opposite directions, and this design should pull it positive.
    ...
    print("    rho(s1, s2) is the informative one: v1's layer-1-only penalty made the "
          "two\n    layers' silence move in OPPOSITE directions. This design should "
          "pull it positive.")
    print(f"{'=' * 70}")
...
def run_milestone() -> None:
    ...
    print(f"{'#' * 70}")
    print(f"# Sparse-network milestone v3 (training): SHD whole, no-delay, "
    ...
    for k in ceiling_k:
        for floor_strength in floor_strengths:
            for seed in seeds:
                run_tag = (
                    f"sparse_{DATASET_KEY}_nodelay_{VERSION_TAG}_"
    ...
    for run_tag, row in summary.items():
        ...
        print(
            f"{run_tag:<58} {row['clean_acc']:>7.4f} "
