"""First milestone v3 (training), NO-DELAY arm: the a x s dissociation factorial.

Successor to [sn_train_noDelay_v2.py](sn_train_noDelay_v2.py). Same network, data,
...
``rho(a, s) = -0.943`` here and ``-0.455`` in the delay arm, so no analysis of v1's
...
Which arm — read this before running it
----------------------------------------
**This is the secondary arm.** The v3 design document nominated it, but Phase 0
reversed that call and the primary venue is now
[sn_train_withDelay_v3.py](sn_train_withDelay_v3.py). Two Phase 0 findings are
properties of *this* arm that the factorial cannot fix, because they concern the
dependent variable rather than the independent ones:

- This arm's readout leaves **+.300** of accuracy unextracted relative to a linear
  decoder on its own 1st hidden layer, on **every** checkpoint (delay arm:
  +.026). Its ``temporal_score`` is as much a statement about ``fc2``/``fc3`` as
  about layer 1.
- Here the deletion control correlates with sparsity at +0.936 against the timing
  probe's +0.939 — a pure *rate* insult and a pure *timing* insult are statistically
  indistinguishable. In the delay arm they separate (+0.713 vs +0.881), and the usage
  effect survives partialling on both ``s`` and the deletion control (+0.716,
  p = .020) where this arm's does not (+0.238, p = .43).

The case *for* running it remains real and is worth the compute if the delay arm
returns a result worth replicating: with no learnable delays, any surviving timing
effect is attributable to the SRM neurons' membrane dynamics alone; the observational
confound is total here (``rho(a, s) = -0.943`` against ``-0.455``), so the factorial
adds the most *manipulation* value; and it is cheaper, ~1.8 h/model against ~2.3 h.

See ``sparse_network_test_progress_v3.md`` §4.
...
- a checkpoint ``sn_data/sparse_whole_nodelay_v3_k{k}_floor{f}_seed{seed}.pt``;
- a training-curve log ``sn_log/..._training_log.json`` carrying the trajectories of
  **both** sparsity axes, not just ``spikes_per_neuron``;
- a row in ``sn_log/sparse_whole_nodelay_v3_train_summary.json`` recording clean test
...
USE_DELAY: bool = False       # no learnable delays; see "Which arm" above
...
#
# Calibrated at k = 1 on the DELAY arm (the primary venue), 400 epochs,
# both floor columns -- inherited here, not re-measured for this arm. 3.0 (v2's value)
...
NATURAL_SPIKES_PER_NEURON: float = 7.85
...
class SparseSHDNetwork(nn.Module):
    """2-hidden-layer SLAYER SNN without delays, for the v3 factorial.

    The parameter set is identical to v1's no-delay class of the same name —
    ``fc1``/``fc2``/``fc3`` weight-norm parameters and nothing else — so v3
...
def run_milestone() -> None:
    ...
    print(f"{'#' * 70}")
    print("# Sparse-network milestone v3 (training): SHD whole, no-delay")
