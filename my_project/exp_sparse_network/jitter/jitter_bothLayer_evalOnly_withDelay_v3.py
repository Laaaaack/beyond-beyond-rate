"""v3 Phase 1 at BOTH LAYERS, WITH-DELAY: jitter sweep, eval-only.
...
accuracy degrades when the trained network's hidden spikes are jittered at
evaluation only.
...
[the no-delay sibling](jitter_bothLayer_evalOnly_noDelay_v3.py). The arms are
...
clipped to ``SUPPORT_BINS[layer]`` — ``[0, 88)`` at layer 1 and ``[0, 160)`` at
...
- writes ``log/sparse_whole_delay_v3L12_jitter_eval.json`` with ``per_setup``,
...
def main() -> None:
    """Load the shared test set once, then sweep jitter over the with-delay arm."""
