...
16 models — which is what makes a jitter curve read against ``a`` interpretable.
...
``jitter_evalOnly_noDelay_v3.py``. The arms are kept as separate scripts (rather
...
- writes ``log/sparse_whole_delay_v3_jitter_eval.json`` mapping each run tag to its
...
def print_summary_table(results: dict, sigma_values: list[int]) -> None:
    """Print one row per checkpoint: both sparsity axes vs clean and jittered accuracy.
