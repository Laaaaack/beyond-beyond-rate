"""First milestone (Step 3), WITH-DELAY arm: 1st-hidden-layer jitter sweep, eval-only.
...
[jitter_evalOnly_noDelay.py](jitter_evalOnly_noDelay.py). The two are kept separate
...
themselves a timing mechanism, so its jitter curve measures sparsity's effect *on
...
- writes ``log/sparse_whole_delay_jitter_eval.json`` mapping each run tag to its
...
def run_jitter_sweep(test_loader: DataLoader) -> dict:
    ...
    print(f"\n{'#' * 70}")
    print("# Sparse-network milestone Step 3 (eval-only jitter, 1st hidden layer)")
...
def print_summary_table(results: dict, sigma_values: list[int]) -> None:
    """Print one row per checkpoint: measured sparsity vs clean and jittered accuracy.

    ``retention`` is the chance-corrected fraction of accuracy surviving the
    strongest jitter; Step 4's ``temporal_score`` is ``1 - retention``. It is
    ...
    Args:
        results: The results dict produced by ``run_jitter_sweep``.
