"""First milestone (Step 3), NO-DELAY arm: 1st-hidden-layer jitter sweep, eval-only.
...
degrades as the **1st hidden layer's** spikes are jittered in time at evaluation
...
[jitter_evalOnly_withDelay.py](jitter_evalOnly_withDelay.py). The two are kept
...
- writes ``log/sparse_whole_nodelay_jitter_eval.json`` mapping each run tag to its
