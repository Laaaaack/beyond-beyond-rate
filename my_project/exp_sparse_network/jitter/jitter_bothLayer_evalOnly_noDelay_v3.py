"""v3 Phase 1 at BOTH LAYERS, NO-DELAY: jitter sweep, eval-only.
...
network's hidden spikes are jittered at evaluation only.
...
[the with-delay sibling](jitter_bothLayer_evalOnly_withDelay_v3.py). The arms
...
Per-spike jitter: each spike at the injected layer is independently shifted by
an offset drawn from ``N(0, sigma)``, clipped to that layer's own measured
temporal support and placed at the nearest free bin. Per-neuron spike count is
preserved, so the perturbation destroys spike *timing*, not *rate* — a network
that leans on timing loses accuracy; a pure rate coder does not.

**Each layer uses its own measured window, and they differ.** Targets are
clipped to ``SUPPORT_BINS[layer]`` — ``[0, 88)`` at layer 1 and ``[0, 90)`` at
...
- writes ``log/sparse_whole_nodelay_v3L12_jitter_eval.json`` with
  ...
  ``acc(sigma)`` sweeps it was computed from. Both carry the training summary's
...
# --- Jitter destination windows, PER LAYER (measured, per arm) ---
# shd_whole.mat holds 100 time bins and load_shd_data zero-pads them to the simulator's
# 200, so neither hidden layer ever occupies the whole window. Clipping targets to T - 1 = 199 lets the largest sigmas
# push spikes into a region where no hidden spike naturally occurs,
...
# §5 step 3). Layer 1's last occupied bin is 87 in both arms; layer 2's is 89 in this arm.
...
def jitter_hidden_batch(
    ...
    Args:
        ...
        support_bins: Upper bound (exclusive) on target bins, i.e. the
            perturbed layer's own measured temporal support. None clips to
...
class SparseSHDNetwork(nn.Module):
    ...
    def forward_with_hidden_perturbation(
        ...
        """Eval-only: jitter spikes at ``site`` before the readout.
        ...
        if sigma > 0 and site in ("l1", "both"):
            hidden1 = jitter_hidden_batch(hidden1, sigma, SUPPORT_BINS[1])
        hidden2 = self._second_hidden(hidden1)
        if sigma > 0 and site in ("l2", "both"):
            hidden2 = jitter_hidden_batch(hidden2, sigma, SUPPORT_BINS[2])
        return self._output(hidden2)
...
def test_at_sigma(
    ...
    """Return test accuracy with ``site`` jittered at ``sigma``.
...
def run_jitter_sweep(test_loader: DataLoader) -> dict:
    """Sweep eval-only jitter at every site and checkpoint, and save.
    ...
    print(f"\n{'#' * 70}")
    print(f"# v3 Phase 1, layers {list(TARGET_LAYERS)} (eval-only jitter)")
...
def print_summary_table(aggregated: dict, sigma_values: list[int]) -> None:
    ...
    jitter; the analysis's ``temporal`` score is ``1 - retention``. It is printed here
