"""v3 Phase 1 at BOTH LAYERS, WITH-DELAY: shift sweep, eval-only.
...
accuracy degrades when the trained network's hidden spikes are shifted at
evaluation only.
...
[the no-delay sibling](shift_bothLayer_evalOnly_noDelay_v3.py). The arms are
...
- writes ``log/sparse_whole_delay_v3L12_shift_eval.json`` with ``per_setup``,
