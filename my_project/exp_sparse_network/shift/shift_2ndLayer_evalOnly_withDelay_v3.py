"""v3 Phase 1 at LAYER 2, WITH-DELAY: 2nd-layer shift sweep, eval-only.
...
when that same layer's spikes are shifted at evaluation only.
...
[the no-delay sibling](shift_2ndLayer_evalOnly_noDelay_v3.py). The arms are
...
``[0, 160)`` would instead pile spikes up at the support edge and merge them,
...
- writes ``log/sparse_whole_delay_v3L2_shift_eval.json`` with two sections:
