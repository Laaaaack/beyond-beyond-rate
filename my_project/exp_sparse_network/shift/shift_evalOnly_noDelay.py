"""First milestone (Step 3), NO-DELAY arm: 1st-hidden-layer shift sweep, eval-only.
...
degrades as the **1st hidden layer's** spikes are shifted in time at evaluation
...
[shift_evalOnly_withDelay.py](shift_evalOnly_withDelay.py). The two are kept
...
- writes ``log/sparse_whole_nodelay_shift_eval.json`` mapping each run tag to its
