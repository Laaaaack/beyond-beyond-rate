"""v3 Phase 1 at BOTH LAYERS, NO-DELAY: relocation sweep, eval-only.
...
network's hidden spikes are relocated at evaluation only.
...
[the with-delay sibling](shd_bothLayer_evalOnly_withDelay_v3.py). The arms are
...
drawn from ``SUPPORT_BINS[layer]`` — ``[0, 88)`` at layer 1 and ``[0, 90)`` at
...
- writes ``log/sparse_whole_nodelay_v3L12_shd_eval.json`` with ``per_setup``,
  ...
  three conditions — and ``per_checkpoint``, the raw per-seed ``acc(f)`` sweeps
