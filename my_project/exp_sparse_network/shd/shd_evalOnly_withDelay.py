"""First milestone (Step 3), WITH-DELAY arm: 1st-hidden-layer spike-relocation sweep, eval-only.
...
[shd_evalOnly_noDelay.py](shd_evalOnly_noDelay.py). The two are kept separate
...
themselves a timing mechanism, so its relocation curve measures sparsity's effect *on
...
- writes ``log/sparse_whole_delay_shd_eval.json`` mapping each run tag to its
