"""v3 Phase 1, NO-DELAY arm: 1st-hidden-layer spike-relocation sweep, eval-only.
...
which is what turns this sweep from a correlation into a test of H1'. See document 5
§2b and §4 Phase 1.

Which arm this is, and what it is for. The sibling is
[shd_evalOnly_withDelay_v3.py](shd_evalOnly_withDelay_v3.py). The arms are kept as
...
neurons' membrane dynamics alone.

This is the sweep whose endpoint carries the headline result, so its two arm-specific
caveats matter more here than anywhere else. Both are from Phase 0, and neither is
fixable by the factorial, because they concern the *dependent* variable rather than
...
- this arm's readout leaves **+.300** of accuracy unextracted relative to a linear
  ...
  the ``usage`` score here is as much a statement about ``fc2``/``fc3`` as about
  layer 1 — which is why the delay arm remains the primary venue for interpretation
  even though this arm carries the larger coefficients;
- its deletion control is strongly tied to selectivity, ``rho(s, control) = -0.818``
  against the delay arm's +0.030, so what this sweep says about ``a`` (VIF 1.03) is on
  much firmer ground than what it says about ``s`` (VIF 3.2).
...
directly, with no delay line in between, so the relocation is applied exactly where
the training penalties acted.
...
- writes ``log/sparse_whole_nodelay_v3_shd_eval.json`` with two sections:
...
the result against the deletion sweep as well — in this arm especially, where
``rho(usage, control) = +0.822``: a network that is simply robust to hidden
interference of any kind scores highly on both.
