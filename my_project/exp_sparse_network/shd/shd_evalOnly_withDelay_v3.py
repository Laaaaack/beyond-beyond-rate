...
16 models — which is what turns this sweep from a correlation into a test of H1'.
See document 5 §2b and §4 Phase 1.

The no-delay v3 arm is calibrated but not yet trained, so it has no sibling script
yet; when it is, it belongs beside this one as ``shd_evalOnly_noDelay_v3.py``. The
arms are kept as separate scripts (rather than one script with a flag) for the same
reason the training scripts are: this network carries the learnable axonal delays
...
- writes ``log/sparse_whole_delay_v3_shd_eval.json`` mapping each run tag to its
  ``acc(f)`` sweep, alongside the training summary's design knobs (``ceiling_k``,
  ``floor_strength``, ``seed``) and *measured* sparsity metrics, so the file is
  self-contained for the analysis.
...
def print_summary_table(results: dict, f_values: list[float]) -> None:
    """Print one row per checkpoint: both sparsity axes vs clean and perturbed accuracy.
