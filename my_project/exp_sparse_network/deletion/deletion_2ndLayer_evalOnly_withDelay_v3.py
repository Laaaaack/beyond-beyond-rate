"""v3 Phase 1 at LAYER 2, WITH-DELAY: 2nd-layer deletion sweep, eval-only.
...
when that same layer's spikes are deleted at evaluation only.
...
[the no-delay sibling](deletion_2ndLayer_evalOnly_noDelay_v3.py). The arms are
...
- writes ``log/sparse_whole_delay_v3L2_deletion_eval.json`` with two sections:
  ...
  conditions — and ``per_checkpoint``, the raw per-seed ``acc(p_d)`` sweeps it
