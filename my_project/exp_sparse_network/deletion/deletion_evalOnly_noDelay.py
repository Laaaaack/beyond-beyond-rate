"""First milestone (Step 3), NO-DELAY arm: 1st-hidden-layer deletion sweep, eval-only.
...
degrades as the **1st hidden layer's** spikes are deleted at evaluation
...
[deletion_evalOnly_withDelay.py](deletion_evalOnly_withDelay.py). The two are kept
...
- writes ``log/sparse_whole_nodelay_deletion_eval.json`` mapping each run tag to its
