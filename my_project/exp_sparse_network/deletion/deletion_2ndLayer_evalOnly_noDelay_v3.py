"""v3 Phase 1 at LAYER 2, NO-DELAY: 2nd-layer deletion sweep, eval-only.

Companion to the training script
[sn_2ndLayer_train_noDelay_v3.py](../sn_2ndLayer_train_noDelay_v3.py), which
trains the **2nd-layer** v3 factorial — a per-pair ceiling ``relu(count - k)``
setting temporal sparsity ``a`` along ``CEILING_K = [1, 2, 4, 8]``, crossed
with a membrane-potential floor setting selectivity ``s`` along
``FLOOR_STRENGTH = [0, 1]``, at seeds 42, 43 and 44 (4 k x 2 floor x 3 seeds =
24 models) — and saves one checkpoint per ``(k, floor, seed)``. **Both
penalties act on the 2nd hidden layer**, so this script performs **no
training** and measures how much test accuracy degrades when that same layer's
spikes are deleted at evaluation only.
...
so until ``sn_log/sparse_whole_nodelay_v3L2_train_summary.json`` exists this
...
is **+0.829** (p = 1.4e-4), the *opposite sign* from layer 1's -0.943. Layer 2
...
[the with-delay sibling](deletion_2ndLayer_evalOnly_withDelay_v3.py). The arms
...
0 numbers — the readout-efficiency gap (+.300 here, +.026 in the delay arm) and
the deletion control being indistinguishable from the timing probe in the
...
This script sweeps the full grid, which shows the *shape* of the degradation
rather than one point of it. Note that ``phase1_measure.py`` is itself still
hardwired to layer 1 and is on document 6 §4 step 3's retarget list; once
retargeted it must share this file's injection site and window for the two to
agree.

Where the perturbation site sits: ``hidden2 = spike(fc2(psp(hidden1)))``,
exactly the tensor the training penalties charged, and what ``fc3`` reads.
There is no delay line anywhere in this arm, so the site is unambiguous — but
see the sibling, where ``delay1`` sits upstream of this layer and must be
applied.
...
Per-spike deletion: each 2nd-hidden spike is dropped independently with
probability ``p_d``; survivors keep their exact times and no spikes are added.
This is the one perturbation here that is *not* rate-preserving — it is the
milestone's rate/robustness control, and its curve is read against the timing
perturbations (relocation, jitter, shift) rather than as a timing measure on
its own.

**This sweep takes no support-window correction** — deletion removes spikes and
never chooses a destination bin, so there is no window to confine and the
layer's temporal support is irrelevant to it. Relocation and jitter take the
per-arm window; shift and deletion do not, and forcing it on them would
introduce the artifact it removes (document 6 §4 step 3, first trap).

Its role is unchanged by the move to layer 2, and Phase 1 at layer 1 showed the
control is not optional: it tracked sparsity at +0.936 (no-delay) and +0.713
(delay), nearly as strongly as the timing probes did, so much of what looks
like a timing effect is a network simply being robust to hidden interference of
any kind. Whether that also holds at layer 2 is one of the things this grid is
for.
...
- sweeps ``PD_VALUES`` on the **2nd** hidden layer, ``NUM_REPEATS`` times per
  p_d for error bars, all inside ``torch.no_grad()``;
- writes ``log/sparse_whole_nodelay_v3L2_deletion_eval.json`` with two
  sections: ``per_setup``, one **seed-averaged** row per ``(k, floor)`` cell —
  the headline, since the three seeds are replicates of one cell rather than
  three conditions — and ``per_checkpoint``, the raw per-seed ``acc(p_d)``
...
The probe's .541-.597 came from four models at 400 epochs, and document 6 is
...
with no learnable delays anywhere. Unchanged from v1 and from the 1st-layer v3
grid — only the layer the penalties and this sweep act on moved, which is why
these checkpoints load with ``strict=True``.
...
# There is no delay line anywhere in this arm, so layer 2 is simply what
# ``fc2`` produces from layer 1's spikes.
TARGET_LAYER: int = 2

# Training generation to evaluate. "v3L2" selects the 24-model **2nd-layer**
# factorial trained by sn_2ndLayer_train_noDelay_v3.py; it matches that script's own
...
# the run tags (``sparse_whole_nodelay_v3L2_k{k}_floor{F}_seed{seed}``) and
...
class SparseSHDNetwork(nn.Module):
    ...
    [sn_2ndLayer_train_noDelay_v3.py](../sn_2ndLayer_train_noDelay_v3.py) —
    ...
    the 1st-layer v3 grid's: only the layer the penalties target moved, never
    the architecture.
    ...
    the tensor both training penalties acted on, and passes the result to
    ``fc3``. The forward pass is split into ``_first_hidden`` / ``_second_hidden``
    / ``_output`` so that site is a named stage rather than an inline expression.
    It is eval-only; nothing here is ever trained.
    """
    ...
    def _second_hidden(self, hidden1: torch.Tensor) -> torch.Tensor:
        """hidden1 -> fc2 -> spike -> 2nd hidden spikes (strictly binary)."""
    ...
    def forward_with_hidden_perturbation(
        ...
        """Eval-only: delete **2nd** hidden layer spikes before ``fc3``.
...
def test_at_pd(
    ...
    """Return test accuracy with spikes deleted from the 2nd hidden layer at ``p_d``.
...
def load_train_summary() -> dict:
    ...
    if not summary_path.exists():
        raise FileNotFoundError(
            ...
            f"sn_2ndLayer_train_noDelay_v3.py first — this script only evaluates "
...
def run_deletion_sweep(test_loader: DataLoader) -> dict:
    """Sweep eval-only 2nd-layer deletion over every checkpoint and save results.
    ...
    print(f"\n{'#' * 70}")
    print(f"# v3 Phase 1, layer {TARGET_LAYER} (eval-only deletion)")
