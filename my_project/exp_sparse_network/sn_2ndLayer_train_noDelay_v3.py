"""2nd-layer milestone v3 (training), NO-DELAY arm: the a x s dissociation factorial.

Sibling of [sn_train_noDelay_v3.py](sn_train_noDelay_v3.py), which runs the identical
factorial against the **1st** hidden layer. Same network, data, splits, optimiser,
schedule and logging; the only difference is **which layer the two penalties act on**,
and therefore which layer's code the experiment is about.
...
network's timing computation lives.

Layer 2 is the natural place to test that, and the baseline measurement says the two
layers are genuinely different objects rather than two copies of the same thing
(``v3_analysis/layer2_baseline.py``, all 27 v1 checkpoints):

=========================  =================  =================
                           layer 1            layer 2
=========================  =================  =================
``spikes_per_neuron``      1.21 - 7.85        3.52 - 5.53
``a`` (per active neuron)  3.06 - 11.49       7.49 - 12.46
``s`` (silent fraction)    31.7% - 60.5%      46.9% - 61.2%
``rho(a, s)`` across v1    **-0.943**         **+0.829**
temporal support           ``[0, 88)``        ``[0, 90)``
=========================  =================  =================

The sign flip in ``rho(a, s)`` is the headline. At layer 1, v1's penalty drove ``a``
down while pushing ``s`` up — the confound the factorial exists to break. At layer 2,
which v1 never penalised directly, the two axes move *together*: as the layer-1 penalty
strengthens, layer 2 becomes both less active per neuron **and** less silent. So layer 2
carries its own confound, running the other way, and at ``+0.829`` it is comfortably
past the ``|rho| <= 0.5`` acceptance threshold. **The factorial is as necessary here as
it was at layer 1, and it is a different confound being broken.**
...
- **Silent pairs must be out of reach of a count-based penalty**, or the potential
  ...
  **0.00 - 0.36** against ``theta = 10`` — as deeply silent as layer 1's, so the dead
  zone that defeated every count-based floor in v1/v2/v2.1/v2.2 is present here too and
  the potential floor is still the right instrument.
- **The floor's gradient must not itself need the surrogate.** At layer 1,
  ...
  vanishes. At layer 2, ``d potential2 / d W2 = psp(hidden1)``, which is likewise
  surrogate-free — **but it is zero for any sample whose entire layer 1 is silent**,
  and such a pair could only be reached back through ``fc1``, i.e. through the very
  surrogate the floor exists to avoid. This failure mode has no layer-1 analogue
  (layer 1's input is data and is never silent). Measured across all 27 checkpoints:
  **0.00% of samples** have an entirely silent layer 1, in either arm. The risk is
  real in principle, absent in this network, and worth re-checking if the ceiling is
  ever driven hard enough to empty layer 1.
...
-----------------------------------------------
**Measured for layer 2 in this arm** — ``NATURAL_SPIKES_PER_NEURON`` (5.53, against
layer 1's 7.85) and the temporal support ``[0, 90)``.
...
3. ``spikes_per_neuron`` not inflated beyond this layer's natural 5.53;
...
this must improve on is **+0.829** in this arm.
...
- a checkpoint ``sn_data/sparse_whole_nodelay_v3L2_k{k}_floor{f}_seed{seed}.pt``;
- a training-curve log ``sn_log/..._training_log.json`` carrying the trajectories of
  **both** sparsity axes, not just ``spikes_per_neuron``;
- a row in ``sn_log/sparse_whole_nodelay_v3L2_train_summary.json`` recording clean test
...
# one seed -- 4 models, ~2.3 h on this arm. Its only job is to validate the
...
#
# Layer 2's natural `a` in this arm is 7.49-12.46 (measured on v1's 15 checkpoints), so
# k = 8 sits just below the bottom of that range and k = 1 is an order of magnitude
# under it. The grid therefore spans the natural rate down to a single spike, which is
# what the row axis needs. Contrast the delay arm, where layer 2's natural `a` is
# 20.71-30.44 and this same grid would leave the top of the axis unanchored -- that
# script adds a k = 16 row for exactly that reason.
...
#
# Two reasons it may need re-tuning here rather than transferring:
#   - layer 2 starts denser in `a` (7.49-12.46 against layer 1's 3.06-11.49), so the
#     ceiling has further to push at every k;
...
# this before widening the k grid.
...
# The natural firing rate of the 2nd hidden layer in this arm, measured on v1's 15
# checkpoints (range 3.52-5.53; this is the densest). The ceiling must not drive the
...
#
# This is a LAYER-2 figure and differs from the 1st-layer script's 7.85. The two layers
# also differ far more in the delay arm (11.66 at layer 1, 23.93 at layer 2), so the
# constant must never be carried across either a layer or an arm boundary.
NATURAL_SPIKES_PER_NEURON: float = 5.53
...
# they opposed each other (-0.943).
V1_LAYER2_AXIS_CORRELATION: float = +0.829
...
class SparseSHDNetwork(nn.Module):
    """2-hidden-layer SLAYER SNN without delays, for the 2nd-layer v3 factorial.
    ...
    def _first_hidden(self, x: torch.Tensor) -> torch.Tensor:
        """input -> fc1 -> spike (no delays anywhere)."""
    ...
    def _second_hidden(
        ...
        discarded. ``d potential2 / d W2 = psp(hidden1)`` is an ordinary convolution
        gradient — no surrogate — which is what lets the floor reach pairs that sit far
        below threshold.
...
def floor_penalty(potential: torch.Tensor,
    ...
    potential of **0.00-0.36** against ``theta = 10`` in this arm — they are not
    ...
    ``d potential2 / d W2 = psp(hidden1)`` is an ordinary convolution gradient and does
    not vanish — with one caveat that is specific to acting on a *hidden* layer rather
    than on the first: it **is** zero for a sample whose entire layer 1 is silent, and
    such a pair is reachable only back through ``fc1``'s surrogate. Measured at 0.00%
    of samples across all 27 v1 checkpoints, so this is a documented edge rather than a
    live problem, but it is the thing to check first if the floor ever stops biting.
