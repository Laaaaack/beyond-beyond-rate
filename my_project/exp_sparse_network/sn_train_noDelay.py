...
Companion to [sn_train_withDelay.py](sn_train_withDelay.py): same dataset,
...
The eval-only timing perturbation (jitter) lives in the companion
``jitter/jitter_evalOnly.py`` and must never be run here (clean training is the protocol).
...
- a checkpoint ``sn_data/sparse_whole_nodelay_tgt{target}_str{strength}_seed{seed}.pt``;
- a training-curve log ``sn_log/..._training_log.json``;
- a row in ``sn_log/sparse_whole_nodelay_train_summary.json`` recording the achieved
