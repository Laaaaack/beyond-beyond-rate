...
TS_MS = 0.5  # Sampling period in ms (2000 Hz sampling rate)
TIME_STEPS = 2000  # 2000 steps * 0.5 ms = 1000 ms total duration
NUM_NEURONS = 10
...
def generate_spike_train(
    ...
    Args:
        ...
        time_steps: Number of discrete time bins in the spike train.
        ts_ms: Width of each time bin in ms (sampling period).
    ...
    duration_seconds = time_steps * ts_ms / 1000.0
    num_pairs = int(np.round(firing_rate * duration_seconds))
    isi_steps = max(1, int(round(isi / ts_ms)))
