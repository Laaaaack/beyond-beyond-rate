...
# Doubled-rate variant: tSample is 2x the original (200) to match the doubled
# SHD time resolution (data T=200) while preserving the delay headroom.
SIM_PARAMS = {"Ts": 1, "tSample": 400}
LIF_PARAMS = {
    "type": "SRMALPHA",
    "theta": 10,
    "tauSr": 1,
...
SEED: int = 42
MAX_DELAY: int = 128  # doubled-rate variant: 2x original (64) keeps the same physical delay range
EARLY_STOP_PATIENCE: int = 300

# Doubled-rate variant: sigmas are 2x the original [0, 1, 3, 5, 10, 17, 25] so
# the physical jitter magnitudes match at the doubled (T=400) time resolution.
SIGMA_VALUES: list[int] = [0, 2, 6, 10, 20, 34, 50]
NUM_REPEATS: int = 3

INPUT_DIM: int = DATASET_CONFIGS[DATASET_KEY]["input_dim"]


def load_shd_data(mat_path: str, target_T: int = 400) -> tuple[np.ndarray, np.ndarray]:
...
class JitterSHDNetwork(nn.Module):
    ...
    def clamp_delays(self, max1: int = 128, max2: int = 128) -> None:
...
def build_loss_and_optimizer(
    ...
    error_cfg = {
        ...
        "training": {
            "error": {
                "type": "NumSpikes",
                "tgtSpikeRegion": {"start": 0, "stop": 400},
...
def run_variation_sweep(
    ...
    # Derive input_dim from the data rather than DATASET_CONFIGS: the filtered
    # SHD variants (norm/part) can change channel count when the sample rate
    # changes (e.g. 224 -> 225 at the doubled rate). X is (N, neurons, T).
    input_dim = X.shape[1]
