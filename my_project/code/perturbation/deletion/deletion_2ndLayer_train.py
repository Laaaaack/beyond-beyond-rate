...
This script implements Experiment 3C from the "Beyond Beyond Rate" project.

We train a single 2-hidden-layer SNN on the SHD dataset with clean
(unperturbed) inputs, then sweep per-spike deletion at the 2nd hidden layer
output during evaluation only. This follows the branch-wide
train-clean / eval-perturbed protocol.

Each spike in the 2nd hidden layer output is independently dropped with
probability p_d. This reduces the total spike count while preserving the
timing of surviving spikes.

Key question: Is the internal representation more or less robust to spike
deletion than the input representation?
...
class DeletionSHDNetwork(nn.Module):
    ...
    def _second_hidden(self, hidden1: torch.Tensor) -> torch.Tensor:
        # (delay1) -> PSP -> fc2 -> spike. Returns raw binary hidden2 spikes (pre-delay2).
        if self.use_delay:
    ...
    def _output(self, hidden2: torch.Tensor) -> torch.Tensor:
        # (delay2) -> PSP -> fc3 -> spike.
        if self.use_delay:
            hidden2 = self.delay2(hidden2)
    ...
    def forward_with_hidden_perturbation(
        ...
        # round-trip in delete_hidden_batch is not autograd-safe.
        x = self._prepare_input(x)
        hidden1 = self._first_hidden(x)
        hidden2 = self._second_hidden(hidden1)
...
# ============================================================
# 8. Testing with Hidden-Layer Spike Deletion
# ============================================================
...
def run_variation(dataset_key: str, use_delay: bool) -> dict:
    ...
    # Evaluate across the deletion sweep
