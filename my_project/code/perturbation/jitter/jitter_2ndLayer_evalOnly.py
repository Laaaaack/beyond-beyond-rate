"""Experiment 3A: Hidden Per-Spike Jitter — 2nd Hidden Layer (eval-only).

Load 2-hidden-layer SNNs that were trained on the clean SHD dataset and sweep
per-spike Gaussian jitter applied at the 2nd hidden layer output at evaluation
time only (train-clean / eval-perturbed protocol). **No training is performed.**

This is the "train once / eval all" companion to ``jitter_2ndLayer_train.py``.
Instead of training a fresh model per jitter level, it loads the single
clean-trained checkpoint ``jitter_2ndLayer_{dataset}_{delay_tag}_sigma0.pt`` for
...
class JitterSHDNetwork(nn.Module):
    ...
    def _apply_jitter(
        ...
        """Apply per-spike jitter to the 2nd hidden layer output (eval-only)."""
        if sigma <= 0:
            return hidden
        return jitter_hidden_batch(hidden, sigma)
...
def load_pretrained_model(
    ...
    """Load a clean-trained JitterSHDNetwork checkpoint ready for evaluation."""
...
def run_variation_sweep(
    ...
    """Load the clean checkpoint and sweep 2nd hidden layer jitter at eval time."""
    ...
    print(f"\n{'#' * 70}")
    print(f"# 2nd Hidden Layer Jitter (eval-only): dataset={dataset_key} | delay={delay_tag}")
    ...
    results_path = LOG_DIR / f"{model_prefix}_evalOnly_jitter_sweep_results.json"
