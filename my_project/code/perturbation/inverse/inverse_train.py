"""Experiment 4A: SHD Time Reversal at Hidden Layer (train-at-f / eval-at-f).

For each perturbation level f and reversal condition (off / on), a fresh
2-hidden-layer SNN is trained from scratch with the intervention active at
the 1st hidden layer output on every batch, then evaluated at the same setting.
"""
...
# When False, only the DATASET_KEY / USE_DELAY settings above are used.
# =====================================================================
...
class SHDNetwork(nn.Module):
    """2-hidden-layer SLAYER SNN for SHD classification with hidden reversal.

    delay1 is applied at the start of _second_hidden_and_output (Option B)
    so the perturbation/reversal hook sees the 1st hidden layer's binary
    spike output directly.
    """
    ...
    def forward(
        ...
        """Forward pass with optional hidden perturbation and time reversal.

        Args:
            x: Input spike trains.
            f: Fraction of hidden spikes to randomly relocate.
            reverse: Whether to time-reverse the hidden spikes.
        ...
        if reverse:
            hidden1 = self._apply_reversal(hidden1)
...
def train_model(
    ...
    """Train the SHDNetwork with hidden perturbation/reversal on every batch.
    ...
    Args:
        ...
        f: Hidden-layer relocation level applied during forward passes.
        reverse: Whether hidden time reversal is active during forward passes.
...
def test_with_repeats(
    ...
    Args:
        ...
        reverse: Whether hidden time reversal is applied.
...
def run_sweep(dataset_key: str, use_delay: bool) -> None:
    ...
    model_prefix: str = f"inverse_{dataset_key}_{delay_tag}"
    ...
    # --- Print sweep summary ---
    print(f"\n=== Hidden-Layer Time Reversal Sweep (SHD {dataset_key}, {delay_tag}) ===")
