"""Experiment 4B: SHD Time Reversal at 2nd Hidden Layer (eval-only).

Load 2-hidden-layer SNNs that were trained on the clean SHD dataset and sweep
partial spike relocation (fraction f) plus time reversal applied at the 2nd
hidden layer output at evaluation time only (train-clean / eval-perturbed
protocol). **No training is performed.**

This is the "train once / eval all" companion to the train-at-f / eval-at-f
version. Because jitter / shift / deletion / inverse all train identical clean
networks, the clean jitter checkpoints
(``../jitter/data/jitter_2ndLayer_{dataset}_{delay_tag}_sigma0.pt``) are reused
here as the single clean-trained SHD model for each configuration.
...
# Clean jitter checkpoints are reused as the shared clean-trained SHD model.
JITTER_DATA_DIR = SCRIPT_DIR / "../jitter/data"
...
class SHDNetwork(nn.Module):
    ...
    def _apply_perturbation(self, hidden: torch.Tensor, f: float) -> torch.Tensor:
        """Apply partial spike relocation to the 2nd hidden layer (eval-only)."""
        if f <= 0:
            return hidden
        return perturb_hidden_batch(hidden, f)

    def _apply_reversal(self, hidden: torch.Tensor) -> torch.Tensor:
        """Apply time reversal to the 2nd hidden layer (eval-only)."""
        return reverse_hidden_batch(hidden)
...
def load_pretrained_model(
    ...
    """Load a clean-trained checkpoint (jitter cache) into an SHDNetwork.

    The jitter / shift / deletion / inverse experiments all train identical
    clean networks, so the jitter ``sigma0`` checkpoint is a drop-in clean
...
def run_sweep(dataset_key: str, use_delay: bool) -> None:
    ...
    Loads the clean jitter checkpoint once and evaluates it across every
    (reversal condition, f) combination — no training is performed.
    ...
    ckpt_path = JITTER_DATA_DIR / f"jitter_2ndLayer_{dataset_key}_{delay_tag}_sigma0.pt"
    ...
    results_path = LOG_DIR / f"{model_prefix}_evalOnly_reversal_sweep_results.json"
...
def main() -> None:
    """Dispatch single or all-variation sweeps depending on EVAL_ALL_VARIATIONS."""
    os.makedirs(LOG_DIR, exist_ok=True)

    if EVAL_ALL_VARIATIONS:
