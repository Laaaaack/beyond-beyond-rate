...
Apply per-neuron jitter to the output spike trains of the 1st hidden layer
during both training and evaluation: a single Gaussian offset is drawn per
neuron and applied to all of that neuron's spikes, preserving intra-neuron ISI
patterns but disrupting cross-channel timing. A fresh model is trained from
scratch for each shift level sigma (train-at-sigma / eval-at-sigma), with the
shift wired through a straight-through estimator so ``fc1`` keeps receiving
gradient.

This is the script (core training) version of ``shift_train.ipynb`` with the
...
def shift_hidden_batch(
    ...
    input tensor's device, avoiding the CPU/numpy round-trip that dominates
    training cost when the shift runs on every batch (Section 3.6.5 / 7.1.3
...
class ShiftSHDNetwork(nn.Module):
    """2-hidden-layer SLAYER SNN with per-neuron shift at the 1st hidden layer.
    ...
    binary spike output directly. ``forward(x, sigma)`` applies the
    per-neuron shift through a straight-through estimator so ``fc1`` still
    receives gradient when training at ``sigma > 0`` (3.2 / 7.1 of the
    fixes doc).
    """
    ...
    def _apply_shift(
        ...
        ``shift_hidden_batch`` runs under ``@torch.no_grad()``, so the
        ...
        layer stays at random init for the whole run at any ``sigma > 0``
        (Section 7.1 of the fixes doc). Forward value is the shifted tensor;
    ...
    def forward(
        ...
        hidden1 = self._apply_shift(hidden1, sigma)
...
def test_with_repeats(
    ...
    The shift kernel draws its offsets from the torch RNG, so each repeat
...
def run_variation_sweep(
    ...
    """Hidden per-neuron shift sweep for one (dataset_key, use_delay) variation.

    Loads the dataset, builds dataloaders, trains one fresh model per sigma
    in SIGMA_VALUES with the shift active during training, evaluates each at
    the same sigma, and persists per-variation checkpoints + JSON.
    ...
    for sigma in SIGMA_VALUES:
        print(f"\n=== Training {model_prefix} at sigma={sigma} ms ===")
...
def main() -> None:
    ...
    if TRAIN_ALL_VARIATION:
        ...
        print(f"Model prefix: shift_{DATASET_KEY}_{tag}")
        print(f"Sigma sweep: {SIGMA_VALUES}")
