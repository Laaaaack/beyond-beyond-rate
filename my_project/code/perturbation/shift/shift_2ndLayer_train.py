"""Experiment 3B: Hidden Per-Neuron Jitter (Shift) — 2nd Hidden Layer.

Apply per-neuron jitter to the output spike trains of the 2nd hidden layer
during both training and evaluation: a single Gaussian offset is drawn per
neuron and applied to all of that neuron's spikes. A fresh model is trained
from scratch for each shift level sigma (train-at-sigma / eval-at-sigma),
with the shift wired through a straight-through estimator.

This script is analogous to ``shift_train.py`` but applies shift to the
...
def train_model(
    ...
    """Train ShiftSHDNetwork with 2nd hidden layer per-neuron shift."""
    set_seed(seed)
...
def run_variation_sweep(
    ...
    """Hidden per-neuron shift sweep for 2nd hidden layer."""
    ...
    print(f"\n{'#' * 70}")
    print(f"# 2nd Hidden Layer Shift: dataset={dataset_key} | delay={delay_tag}")
