"""Experiment 2B: SSC — 2nd Hidden Layer Perturbation.

Train a 2-hidden-layer SNN on the Spiking Speech Commands (SSC) dataset with
perturbation active on the output of the 2nd hidden layer during training
(through a straight-through estimator), then evaluate at the same perturbation level.

This script is analogous to ``ssc_train.py`` but applies perturbation to the
...
def load_ssc_data(
    ...
    """Load SSC dataset from HDF5 file with zero-padding to target_T."""
...
def train_model(
    ...
    """Train SSCNetwork with 2nd hidden layer perturbation."""
    set_seed(seed)
...
def main() -> None:
    if TRAIN_ALL_VARIATION:
        ...
        for ds, ud in ALL_VARIATIONS:
            ...
            print(f"  - ssc_2ndLayer_{ds}_{tag}")
    else:
        ...
        print(f"Model prefix: ssc_2ndLayer_{DATASET_KEY}_{tag}")
