"""SSC dataset hidden-layer spike-timing perturbation training script.

Experiment 2B: Train SNNs on SSC dataset (no perturbation), then evaluate
with spike-timing perturbation applied at the 1st hidden layer output.
...
class SSCNetwork(nn.Module):
    ...
    def _second_hidden_and_output(self, hidden1: torch.Tensor) -> torch.Tensor:
        """hidden1 -> PSP -> fc2 -> spike -> PSP -> fc3 -> spike."""
    ...
    def forward_with_hidden_perturbation(
        ...
        """Forward pass with perturbation at 1st hidden layer.
...
def test_with_hidden_perturbation(
    ...
    """Evaluate accuracy with hidden-layer perturbation.
...
def main() -> None:
    ...
    for dataset_key, use_delay in config_grid:
        ...
        print(f"\n  --- Hidden-perturbation sweep at evaluation ---")
