"""Experiment 2A: SHD — Eval-only Sweep (1st Hidden Layer Perturbation).
...
This is the evaluation-only counterpart of ``shd_evalOnly``'s original
train-at-f / eval-at-f protocol: it reuses the persisted checkpoints and
re-runs the perturbed test sweep for every requested dataset/delay
variation, persisting the sweep results.
"""
