"""Experiment 2B: SSC — Eval-only Sweep (1st Hidden Layer Perturbation).

For each perturbation level *f* the 2-hidden-layer SNN checkpoint that was
trained at that *f* (with 1st hidden-layer perturbation) is loaded from
``data/`` and evaluated at the same *f*. No training is performed.

This is the evaluation-only counterpart of the train-at-f / eval-at-f
protocol: it reuses the persisted checkpoints and re-runs the perturbed test
sweep for every requested dataset/delay variation, persisting the sweep
results.
"""
...
# =====================================================================
# Batch mode: when True, the main run evaluates EVERY combination of
...
# the fixed-weight checkpoints the 1st hidden layer stops at bin 87 in both arms,
...
def load_model(
    ...
    """Instantiate an SSCNetwork and load weights from a checkpoint.
...
def run_variation_sweep(
    ...
    """Eval-only sweep for one (dataset_key, use_delay) variation.
    ...
    evaluates it at the same f. No training is performed. Persists the
    per-variation eval results as JSON.

    Args:
        dataset_key: One of "norm", "part", "whole".
        use_delay: Evaluate the SGD-delay variant if True, else SGD.

    Returns:
        Dict with keys models / results / test_loader / model_prefix
...
def main() -> None:
    """Run the configured eval-only sweep(s)."""
